...
begin
    ...
    if ContentLength > 0 then
      begin
        // Check if we have gone past the expected Content Length
        // Multipart responses have a boundary marker at the end which we REALLY don't want!
        if FRcvdCount > ContentLength then
          begin
            // Yep we've gone over - ignore the last bit beyond what we want!
            ExcessBytes:= FRcvdCount - ContentLength;
            Len:= Len - ExcessBytes;
            FRcvdCount:= FRcvdCount - ExcessBytes;
            if Len <= 0 then
              exit;
          end;
      end;
...
begin
    ...
    if FLastResponse = '' then begin
        if FBoundaryFlag then begin
            // Haven't reached the boundary, so skip this blank line!
            exit;
        end;
    ...
    if FBoundaryFlag then
      begin
        if Pos(FBoundary, LowerCase(FLastResponse)) > 0 then
          begin
            FBoundaryFlag:= False;
//            exit;
          end;
    ...
    if (Copy(FLastResponse, 1, 8) = 'HTTP/1.0') or
       (Copy(FLastResponse, 1, 8) = 'HTTP/1.1') then begin
        ...
        if Field = 'location' then begin { Change the URL ! }
          ...
          begin
            nPos:= Pos('boundary=', LowerCase(Data));
            if nPos > 0 then
              begin
                nPos:= nPos + Length('boundary=');
                FBoundary:= LowerCase(Copy(Data, nPos, Length(Data) - (nPos - 1)));
                FBoundaryFlag:= True;
              end;
